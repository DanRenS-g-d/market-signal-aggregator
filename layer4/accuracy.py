...
def get_accuracy_report() -> dict:
    ...
    report = {
        ...
        "avg_pnl_pct": round(float(avg_pnl), 3) if avg_pnl else 0,
        "best_trade_pct": round(float(best), 3) if best else 0,
        "worst_trade_pct": round(float(worst), 3) if worst else 0,
        "by_pair": [
            {
                "pair": r[0],
                "trades": r[1],
                ...
                "avg_pnl": round(float(r[3]), 3) if r[3] else 0,
            }
            for r in pair_rows
        ],
